...
trunkopened = 1;
if(dialog) exitWith {trunkopened = 0;};
_vehicle = [_this,0,Objnull,[Objnull]] call BIS_fnc_param;
if(isNull _vehicle OR !(_vehicle isKindOf "Motorcycle" OR _vehicle isKindOf "Car" OR _vehicle isKindOf "Air" OR _vehicle isKindOf "Ship" OR _vehicle isKindOf "House_F" OR _vehicle isKindOf "Land_CargoBox_V1_F")) exitWith {trunkopened = 0;}; //Either a null or invalid vehicle type.
...
closeDialog 0;
if((_vehicle getVariable ["trunk_in_use",false])) exitWith {hint localize "STR_MISC_VehInvUse"; trunkopened = 0;};
