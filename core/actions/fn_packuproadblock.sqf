...
{	
	...
	if(([true,"RoadBlockConc",1] call life_fnc_alrphandleinventory)) then
	{
	titleText["You have packed up a road block.","PLAIN"];
	{deleteVehicle _x} foreach _roadblockobjects;
	};
};
	case ((typeOf _roadblock) == "RoadCone_F") : {
	_roadblockobjects = _roadblock getVariable "roadblock";
	if(([true,"RoadBlockWood",1] call life_fnc_alrphandleinventory)) then
	{
	...
	_roadblockobjects = _roadblock getVariable "roadblock";
		if(([true,"RoadBlockRebel",1] call life_fnc_alrphandleinventory)) then
	{
