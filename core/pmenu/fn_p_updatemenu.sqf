...
{ if(player distance _x < 10) then {_near_units set [count _near_units,_x];};} foreach playableUnits;
{
	...
	{
		_near lbAdd format["%1 %2",_x getVariable["realname",name _x]];
		_near lbSetData [(lbSize _near)-1,str(_x)];
		_near_i lbAdd format["%1 %2",_x getVariable["realname",name _x]];
		_near_i lbSetData [(lbSize _near)-1,str(_x)];
	};
} foreach _near_units;

_mstatus ctrlSetStructuredText parseText format["<t size='1.2px'>$%1</t>",[thisisdefinatenlynotafuckingniggercashvariableyoucoonfejfiejfejifeijefijefiejfiejefijfjei] call life_fnc_numberText];
_mstatus2 ctrlSetStructuredText parseText format["<t size='1.2'>$%1</t>",[bradleyisacoonniggerfeiojfijwefweijoijofewiojfeiuowfeiojfewiojwefijofewjiowefijowfejiowfe] call life_fnc_numberText];
