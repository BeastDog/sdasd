...
while {Life_Lawyer == 1} do {
	...
	if (_count == 0) exitWith { ["<t size='3.2' color='#04EE4A'>Your job contract expired! Feel free to sign up again.</t>"] call life_fnc_alert; JobType = 0; Life_Lawyer = 0; Life_Working = 0;player setUnitLoadout lawyer;};	
		if (player distance getMarkerpos "jail_marker" < 500) then {["<t size='3.2' color='#04EE4A'>A paycheck of $900 has arrived in your bank, head down to your local bank to collect it.</t>"] call life_fnc_alert;paycheckzwdqdwqdqwdqwgegrihorghrgwdwdwddwqdwdwefgwuohweijohwefweijwefijofewijo = paycheckzwdqdwqdqwdqwgegrihorghrgwdwdwddwqdwdwefgwuohweijohwefweijwefijofewijo + 9; };
};
