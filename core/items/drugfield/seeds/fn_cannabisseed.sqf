...
if(life_seeded == 10) exitWith {hint "You already have seeds planted.";[true,"cannabis seed",1] call life_fnc_alrphandleinventory; };
if(life_shovel == 0) exitWith {hint "You have not cultivated the land.";[true,"cannabis seed",1] call life_fnc_alrphandleinventory; };
if ((count _lights) == 0) exitWith {hint "You are not close enough to the lights";[true,"cannabis seed",1] call life_fnc_alrphandleinventory; };
...
uiSleep 4;
["<t size='3.2' color='#04EE4A'>Cannabis seeds usually take around 100 seconds to grow.</t>"] call life_fnc_alerttwo;
...
[true,"cannabis",_amount] call life_fnc_alrphandleinventory;
uiSleep 3;
