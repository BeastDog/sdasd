...
if(!_isVehicle) then {
	...
	_dice = random(150);
	_lock1 = [life_currentExpPerks, "Perk4_4"] call mav_ttm_fnc_hasPerk; 
	_lock2 = [life_currentExpPerks, "Perk4_4_1"] call mav_ttm_fnc_hasPerk; 
	_lock3 = [life_currentExpPerks, "Perk4_4_1_1"] call mav_ttm_fnc_hasPerk; 
	if (_lock1) then {
	_dice = random(128);
	};
	if (_lock2) then {
