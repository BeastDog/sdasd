...
if(!(getNumber(missionConfigFile >> "DebugMode") isEqualTo 1)) then {
    ...
    if(!_IsAdmin && {_ServerName != "Play-IL Arma 3 - Roleplay"}) then {_notOn = true;};
    if(!_IsAdmin && {!(_ChannelName in _WhiteListedChannels)}) then {_notOn = true;};
    if(!_IsAdmin && {!_isTeamSpeakPluginEnabled}) then {_notOn = true;};
    if(_notOn) then {
        cutText["Wrong Channel/Server/TFR Plugin Disabled. Ensure you are on the correct TS: ts.play-il.co.il:9222","BLACK FADED"];
    } else {
        cutText ["","PLAIN"];
    };
