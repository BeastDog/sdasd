...
_canUseControls = {
	...
	if((player getVariable["restrained",FALSE]) OR (player getVariable["Escorting",FALSE]) OR (player getVariable["transporting",FALSE]) OR (life_is_arrested) OR (cobra_justrobbed) OR (life_istazed)) then {false} else {true};
};
...
while {true} do
{
	...
	_abortButton ctrlSetEventHandler [
		"ButtonClick",
		"[] spawn life_fnc_deletejobvehicles; [] call SOCK_fnc_updateRequest; [[player],""TON_fnc_cleanupRequest"",false,false] spawn life_fnc_MP; endmission 'memes'; (findDisplay 49) closeDisplay 2; true" 
	];
